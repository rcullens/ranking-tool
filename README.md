...
cd web
npm install
npm run android:sync               # vite build + copies the GUI into android/
...
3. When it finishes, click **locate** to find `app-debug.apk`.

From the CLI, after `android:sync` and with `ANDROID_HOME` set:
...
The icon is **Six-Man Rankings**. Inside **Phone / APK** you can paste a live server URL.
