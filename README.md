...
**Public API (phone What-if / Ingest, no PC):** **https://ranking-tool-rlrv.vercel.app**

- `GET https://ranking-tool-rlrv.vercel.app/api/health` → `{"ok": true}`
- `POST https://ranking-tool-rlrv.vercel.app/api/what-if` and `POST /api/ingest`
- Same app also deploys at `https://ranking-tool-rlrv.vercel.app` (second Vercel git project on this repo)
...
What-if / webhook ingest hit the public FastAPI host **https://ranking-tool-rlrv.vercel.app** (the existing Vercel git link for this repo, now the engine backend). **Phone / APK → Live server URL** is prefilled with that origin — no PC and no manual paste. Override it only for a LAN `sixman-rank serve`, or tap **Use bundled data** for snapshots only.
...
3. What-if is ready: the live server URL is already **https://ranking-tool-rlrv.vercel.app**. Sync still pulls the cron-updated Pages JSON.
...
The icon is **Six-Man Rankings**. The live URL prefills **https://ranking-tool-rlrv.vercel.app**. Tap **Use bundled data** to stay on the snapshot inside the APK.

### Live scores from the phone

The APK cannot run the Python engine. What-if / ingest use **https://ranking-tool-rlrv.vercel.app** by default (same public API as Pages). No LAN serve and no PC.
...
The public host **https://ranking-tool-rlrv.vercel.app** is `sixman-rank serve` on the existing Vercel git project (`app.py` → `sixman_rankings.web.app:app`). Season state is the committed UIL field (refreshed when `live-scores.yml` pushes `main`); in-memory ingest is ephemeral across serverless cold starts. Local `sixman-rank serve` (port 43127) still polls a JSON feed / webhook:
